...
## Чем это отличается от LLM-вики и RAG

LLM-вики хранит знания проекта. RAG находит подходящие фрагменты контекста для модели. Эти системы полезны, и Operation Map может использовать найденные ими материалы как доказательства и источники.

Но сами по себе они не проверяют, что путь реализации продукта действительно полный и связный.

| LLM-вики / RAG | TASKPLAN PRO Operation Map |
|---|---|
| Находит и возвращает знания | Проверяет связную модель выполнения работы |
| Организует документы и контекст | Организует блоки, шаги, артефакты, решения и точки проверки |
| Отвечает «что мы знаем?» | Отвечает «что происходит дальше, зачем и как принять результат?» |
| Может вернуть неполный или устаревший контекст | Явно сообщает о пропущенных элементах и разрывах |
| Работает как интерфейс к знаниям | Работает как контракт между концепцией, командой, агентами и интерфейсом |

Operation Map не заменяет проектную вики, RAG, Git, тесты или решения продуктовой команды. Он добавляет недостающий операционный слой между знаниями о продукте и его реализацией.
